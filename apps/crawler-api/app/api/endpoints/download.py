...
@router.get("/download", summary="在线下载抖音|TikTok|小红书|快手|B站|微博内容/Online download Douyin|TikTok|Xiaohongshu|Kuaishou|Bilibili|Weibo content")
async def download_file_hybrid(request: Request,
    ...
    # [中文]
    ### 用途:
    - 在线下载抖音|TikTok|小红书|快手|B站|微博 无水印或有水印的视频/图片
    ...
    # [English]
    ### Purpose:
    - Download Douyin|TikTok|Xiaohongshu|Kuaishou|Bilibili|Weibo video/image with or without watermark online.
    ...
    try:
        ...
        if data_type == 'video':
            ...
            # 获取视频文件
            __headers = await HybridCrawler.get_platform_headers(platform)

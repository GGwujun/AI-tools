...
@router.get("/video_data", response_model=ResponseModel, tags=["Hybrid-API"],
            summary="混合解析单条内容接口/Hybrid parsing single content endpoint")
async def hybrid_parsing_single_video(request: Request,
    ...
    ### 用途:
    - 该接口用于解析抖音/TikTok/小红书/快手/B站/微博单条内容的数据。
    ### 参数:
    - `url`: 视频链接、分享链接、分享文本。
    ### 返回:
    - `data`: 视频数据。
    ...
    # [English]
    ### Purpose:
    - This endpoint is used to parse data of a single Douyin/TikTok/Xiaohongshu/Kuaishou/Bilibili/Weibo post.
    ### Parameters:

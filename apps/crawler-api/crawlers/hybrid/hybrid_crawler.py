...
from crawlers.bilibili.web.web_crawler import BilibiliWebCrawler  # 导入B站Web爬虫
...
from crawlers.wechat_channels.web.web_crawler import WechatChannelsWebCrawler  # 导入视频号Web爬虫
...
class HybridCrawler:
    def __init__(self):
        ...
        self.WeiboWebCrawler = WeiboWebCrawler()
        self.XiaohongshuWebCrawler = XiaohongshuWebCrawler()
        self.WechatChannelsWebCrawler = WechatChannelsWebCrawler()

    async def get_platform_headers(self, platform: str):
        if platform == "bilibili":
            return await self.BilibiliWebCrawler.get_bilibili_headers()
        if platform == "douyin":
            return await self.DouyinWebCrawler.get_douyin_headers()
        if platform == "kuaishou":
            return await self.KuaishouWebCrawler.get_kuaishou_headers()
        if platform == "tiktok":
            return await self.TikTokWebCrawler.get_tiktok_headers()
        if platform == "weibo":
            return await self.WeiboWebCrawler.get_weibo_headers()
        if platform == "xiaohongshu":
            return await self.XiaohongshuWebCrawler.get_xiaohongshu_headers()
        if platform == "wechatChannel":
            return await self.WechatChannelsWebCrawler.get_wechat_channels_headers()
        return None
    ...
    async def hybrid_parsing_single_video(self, url: str, minimal: bool = False):
        ...
        elif "b23.tv" in url or "bilibili.com" in url:
            platform = "bilibili"
            data = await self.BilibiliWebCrawler.fetch_unified_video_by_url(url)
            aweme_id = data.get("aweme_id")
            aweme_type = 0
        elif "weibo.com" in url or "weibocdn.com" in url:
            platform = "weibo"
            data = await self.WeiboWebCrawler.fetch_one_post(url)
            aweme_id = data.get("post_id")
            aweme_type = 150 if data.get("type") == "image" else 0
        elif "kuaishou" in url or "gifshow" in url or "chenzhongtech" in url:
            platform = "kuaishou"
            data = await self.KuaishouWebCrawler.fetch_one_post(url)
            aweme_id = data.get("photo_id")
            aweme_type = 150 if data.get("type") == "image" else 0
        # 解析TikTok视频/Parse TikTok video
            ...
            data = await self.XiaohongshuWebCrawler.fetch_one_note(url)
            aweme_id = data.get("note_id")
            aweme_type = 150 if data.get("type") == "image" else 0
        elif "weixin.qq.com/sph" in url or "channels.weixin.qq.com" in url:
            platform = "wechatChannel"
            data = await self.WechatChannelsWebCrawler.fetch_one_post(url)
            aweme_id = data.get("object_id")
            aweme_type = 150 if data.get("type") == "image" else 0
        else:
        ...
        if platform in ("xiaohongshu", "kuaishou", "weibo"):
            common_cover_data = {
                'cover': data.get('cover'),
                'origin_cover': data.get('cover'),
                'dynamic_cover': data.get('cover')
            }
        elif platform == "bilibili":
            common_cover_data = data.get('cover_data') or {
                'cover': data.get('cover'),
                'origin_cover': data.get('cover'),
                'dynamic_cover': data.get('cover')
            }
        ...
        if platform == 'bilibili':
            ...
            result_data['extra'] = data.get('extra')
        elif platform == 'weibo':
            if url_type == 'video':
                video_url = (data.get('video_urls') or [None])[0]
                api_data = {
                    'video_data': {
                ...
                image_list = data.get('images') or []
                api_data = {
                    'image_data': {
                        'no_watermark_image_list': image_list,
                        'watermark_image_list': image_list
                    }
                }

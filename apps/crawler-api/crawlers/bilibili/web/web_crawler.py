...
class BilibiliWebCrawler:
    ...
    async def resolve_url(self, url: str) -> str:
        raw_url = self.normalize_input_url(url)
        kwargs = await self.get_bilibili_headers()
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            response = await crawler.fetch_response(raw_url)
        return str(response.url)

    def normalize_input_url(self, raw_input: str) -> str:
    ...
    def extract_bv_id(self, url: str) -> str:
        ...
        for key in ("bvid", "bv_id"):
            values = query.get(key, [])
            if values and values[0]:
                return values[0]

        decoded_path = unquote(parsed.path or "")
        match = re.search(r"(BV[0-9A-Za-z]+)", decoded_path)
        return match.group(1) if match else ""

    async def fetch_one_video_by_url(self, url: str) -> dict:
        resolved_url = await self.resolve_url(url)
        bv_id = self.extract_bv_id(resolved_url)
        if not bv_id:
            raise ValueError("BilibiliWebCrawler: Cannot extract BV id from the URL.")

        response = await self.fetch_one_video(bv_id)
    ...
    async def fetch_unified_video_by_url(self, url: str) -> dict:
        detail_payload = await self.fetch_one_video_by_url(url)
        resolved_url = detail_payload["resolved_url"]
        bv_id = detail_payload["bvid"]
        data = detail_payload["data"] or {}
        ...
        return {
            ...
            "author": {
                "nickname": (data.get("owner") or {}).get("name") or "",
                "unique_id": (data.get("owner") or {}).get("mid") or "",
                "avatar": (data.get("owner") or {}).get("face") or "",
            },
            "music": {},
            "statistics": data.get("stat") or {},
            "cover_data": {
                "cover": data.get("pic"),
